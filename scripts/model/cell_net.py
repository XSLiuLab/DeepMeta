...
def get_cell_net(edge_list,feat,exp,data_type,random=False):
...
class cellNetDataset(Dataset):
    def __init__(self, root, filename, exp, data_type, net_path, cores, random = False,transform=None, pre_transform=None):
        """
    ...
    def _save_data(self, key_value, exp, net_path, data_type, random):
        cell = key_value["cell"]
        ...
        pyg_net = get_cell_net(dt,dt_fea,dt_exp,data_type,random=random)
        pyg_net.cell = cell
